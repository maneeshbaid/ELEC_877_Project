...
class DataProcessor:
    ...
    def _split_dataset(self, X_text, y, texts_original):
        ...
        logger.info(f"Splitting data: Total samples = {len(y)}, Test Ratio={test_size}, Val Ratio (of Train)={val_size}")
        if len(y) < 10:
...
def create_cnn_lstm_model(config, processor):
    logger.info("Building the CNN-LSTM model")

    best_hp = {
        'trainable_embedding': False,
        'conv_filters': 128,
        'conv_kernel_size_k4': 3,
        'conv_kernel_size_k3': 3,
        'lstm_units_1': 40,
        'lstm_units_2': 30,
        'lstm_dropout': 0.3,
        'dense_units_1': 128,
    ...
    conv1 = Conv1D(filters=best_hp['conv_filters'], kernel_size=best_hp['conv_kernel_size_k4'], padding='same', activation='relu',
                   kernel_regularizer=l2(best_hp['l2_reg']), name='conv1d_k4')(embedding_layer)
    conv2 = Conv1D(filters=best_hp['conv_filters'], kernel_size=best_hp['conv_kernel_size_k3'], padding='same', activation='relu',
                   kernel_regularizer=l2(best_hp['l2_reg']), name='conv1d_k3')(embedding_layer)
    max_pool1 = GlobalMaxPooling1D(name='globalmaxpool_k4')(conv1)
    avg_pool1 = GlobalAveragePooling1D(name='globalavgpool_k4')(conv1)
    max_pool2 = GlobalMaxPooling1D(name='globalmaxpool_k3')(conv2)
    avg_pool2 = GlobalAveragePooling1D(name='globalavgpool_k3')(conv2)

    lstm_branch = Bidirectional(LSTM(units=best_hp['lstm_units_1'], return_sequences=True,
    ...
    lstm_branch = Bidirectional(LSTM(units=best_hp['lstm_units_2'], return_sequences=False,
                                       kernel_regularizer=l2(best_hp['l2_reg']), recurrent_regularizer=l2(best_hp['l2_reg']),
                                       dropout=best_hp['lstm_dropout'], recurrent_dropout=best_hp['lstm_dropout'],
                                       name='bilstm_2'))(lstm_branch)

    combined_text_features = Concatenate(name='concat_text')([
        max_pool1, avg_pool1,
        max_pool2, avg_pool2,
        lstm_branch
    ])
    combined_text_features = Dropout(best_hp['dropout_rate_dense'], name='dropout_text_combined')(combined_text_features)

    x = Dense(best_hp['dense_units_1'], activation='relu', kernel_regularizer=l2(best_hp['l2_reg']), name='dense_1')(combined_text_features)
    ...
    model = Model(inputs=text_input, outputs=output, name='FakeNews_CNN_LSTM_Model')
